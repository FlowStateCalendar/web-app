...
export function UserBar({
  ...
  async function handleSaveName() {
    ...
    try {
      ...
      setProfileError(e instanceof Error ? e.message : "Could not save name.");
    } finally {
      setSavingName(false);
    }
  }
    ...
    <>
      <header className="sticky top-0 z-10 flex items-center justify-between border-b border-gray-200 bg-white/95 px-4 py-3 backdrop-blur" data-tour="user-bar">
        <button
          ...
          aria-label={`Open profile (${displayName.split("@")[0]})`}
          ...
          <div className="w-full max-w-sm max-h-[90vh] overflow-y-auto rounded-xl bg-white p-6 shadow-lg">
            ...
            <div className="space-y-4 text-sm">
              ...
              <ProfilePicturePicker
                initialProfilePicture={localProfile?.profile_picture ?? null}
                onSaved={(key) =>
                  setLocalProfile((p) => (p ? { ...p, profile_picture: key } : p))
                }
