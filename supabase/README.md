...
- **`functions/dispatch-reminders/`** – Cron-invoked: sends email (Resend), Web Push (VAPID), and APNs for upcoming task events. See [docs/EDGE-FUNCTIONS-IMPLEMENTATION.md](../docs/EDGE-FUNCTIONS-IMPLEMENTATION.md). Schedule via [manual/dispatch-reminders-cron.sql](manual/dispatch-reminders-cron.sql).
...
Ensure `SUPABASE_URL`, `SUPABASE_ANON_KEY`, and `SUPABASE_SERVICE_ROLE_KEY` are set in the Edge Function secrets (or project env). For cron functions, set **CRON_SECRET** so only your cron jobs can invoke them.
...
- **Dispatch reminders:** Deploy `dispatch-reminders`, set Resend/APNs/VAPID secrets as documented, then run [manual/dispatch-reminders-cron.sql](manual/dispatch-reminders-cron.sql) (every 5 minutes). Requires pg_net.
...
- Run migrations for `last_feed`, weekly leaderboard cron, and notification tables (`20250411000001_notification_channels_and_push.sql`); schedule daily-aquarium-upkeep and dispatch-reminders via SQL Editor with your project ref and CRON_SECRET.
